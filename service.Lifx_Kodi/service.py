...
def compareLightState(sceneStateLights):
    ...
    changeLights = True
    currentState = getLightState(type=bulbs['type'], name=bulbs['set'])
...
def restoreLights(LightStates, duration=5):
    ...
    option = {'states':[]}
    #build the light states for each bulb.
    for light in LightStates:
        option['states'].append({'selector':light['id'],'color':'hue:' + str(light["hue"]) + ' saturation:' + str(light['saturation']) + ' kelvin:' + str(light['kelvin']),'brightness':light['brightness'],'duration':duration,'power':light['power']})
        _log(str(option))
